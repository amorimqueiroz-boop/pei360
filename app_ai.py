...
st.set_page_config(page_title="PEI 360º", page_icon="📘", layout="wide", initial_sidebar_state="expanded")
...
:root { --brand-blue: #004E92; --brand-coral: #FF6B6B; --bg-gray: #F7FAFC; --card-radius: 16px; }
...
.interactive-card:hover { transform: translateY(-3px); border-color: var(--brand-blue); box-shadow: 0 8px 15px rgba(0,78,146,0.08); }
.header-content { display: flex; align-items: center; gap: 25px; }
.stTabs [data-baseweb="tab-list"] { gap: 10px; padding-bottom: 10px; }
.stTabs [data-baseweb="tab"] { height: 45px; border-radius: 25px; padding: 0 25px; background-color: white; border: 1px solid #E2E8F0; font-weight: 700; color: #718096; }
...
.icon-box { width: 45px; height: 45px; background: #EBF8FF; border-radius: 12px; display: flex; align-items: center; justify-content: center; margin-bottom: 15px; color: var(--brand-blue); font-size: 22px; }
...
def f6(d, a):
    ...
    if d['rede_apoio'] or d['orientacoes_especialistas']:
        ...
        pdf.multi_cell(0, 6, f4(f"Profissionais: {p}.\nOrientações: {o}"))
    if d['ia_sugestao']: pdf.ln(5); pdf.multi_cell(0, 6, f4(d['ia_sugestao']))
...
logo_path = f1(); b64_logo = f2(logo_path); mime = "image/png" if logo_path and logo_path.endswith("png") else "image/jpeg"
...
with tab1:
    ...
    with st.expander("📎 Anexar Laudo (PDF)"):
        ...
        if up: st.session_state.pdf_text = f3(up); st.success("PDF Anexado!")
        ...
        if st.button("GERAR PLANO", type="primary"):
            if not st.session_state.dados['nome']: st.error("Preencha o Nome.")
                ...
                with st.spinner("Processando..."):
                    res, err = f5(api_key, st.session_state.dados, st.session_state.pdf_text)
                    if err: st.error(err)
...
with tab6:
    ...
    if st.session_state.dados['ia_sugestao']:
        ...
        with c_pdf: pdf_bytes = f6(st.session_state.dados, a); st.download_button("📥 Baixar PDF", pdf_bytes, f"PEI_{st.session_state.dados['nome']}.pdf", "application/pdf", type="primary")
        with c_word: docx_bytes = f7(st.session_state.dados); st.download_button("📥 Baixar Word", docx_bytes, f"PEI_{st.session_state.dados['nome']}.docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document")
